...
df <- read.csv(paste(d.dir, "tidy", "GB_BOSS_fwd_fine_bathy_habitat_dominant_broad.csv", sep='/')) %>%
...
scatterplotMatrix(df2[3:11], col = df2$Class)

plot(df2[3:11], col = df2$Class)
legend("center", 
...
mosthighlycorrelated(df2[3:11], 10) # This results in only depth, rough and slope 4 not being correlated above 0.95
...
roc_imp <- filterVarImp(x = train[, 3:11], y = train$Class)
roc_imp
...
lp2
#print(lp2)
trellis.device(device ="png", filename = paste(p.dir, "BOSS-fwd-fine.png", sep='/'), width = 1000, height = 670, res = 200)
print(lp2)
dev.off()

# save prediction ---
writeRaster(testx, paste(o.dir, "GBpred-Fine-BOSS-fwd.tif", sep='/'))

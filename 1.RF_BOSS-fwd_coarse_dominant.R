...
df <- read.csv(paste(d.dir, "tidy", "GB_BOSS-fwd_coarse_dominant_broad.csv", sep='/'))
head(df)
...
mosthighlycorrelated(df2[2:10], 12) # This results in only depth, rough and slope 4 not being correlated above 0.95
...
lp2
#print(lp2)
trellis.device(device ="png", filename = paste(p.dir, "BOSS-fwd-coarse.png", sep='/'), width = 1000, height = 670, res = 200)
print(lp2)
dev.off()

# save prediction ---
writeRaster(testx, paste(o.dir, "GBpred-coarese-BOSS-fwd.tif", sep='/'))
